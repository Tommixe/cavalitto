...
{capture name=path}
    <li><a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" title="{l s='Go back to the Checkout' mod='bankwire'}">{l s='Checkout' mod='bankwire'}</a></li><li class="navigation-pipe">{$navigationPipe}</li><li><span>{l s='Bank-wire payment' mod='bankwire'}</span></li>
{/capture}
...
{else}
    <form action="{$link->getModuleLink('bankwire', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
        ...
        <p class="cart_navigation clearfix" id="cart_navigation">
        	<a class="button-exclusive btn btn-default pull-left" href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}">
