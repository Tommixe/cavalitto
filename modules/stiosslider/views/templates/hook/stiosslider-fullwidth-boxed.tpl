...
{if $slide['location']==14}<div class="iosslider_wide_container wide_container {if $slide['hide_on_mobile']} hidden-xs {/if} ">{/if}
<div id="iosSlider_containerOuter_{$slide.id_st_iosslider_group}" class="containerOuter_fullwidth_boxed {if $slide['location']!=14 && $slide['hide_on_mobile']} hidden-xs {/if} iosSlider_tb{$slide['padding_tb']} block">
<div id="iosSlider_{$slide.id_st_iosslider_group}" class="iosSlider fullwidth_default_boxed ">
    <div class="slider clearfix">
        {assign var="selectorsBlock" value=""}
        {foreach $slide['slide'] as $banner}
            <div id="iosSliderBanner_{$banner.id_st_iosslider}" style="height:{if $slide.height}{$slide.height}{else}500{/if}px;" class="iosSliderBanner_{$banner.id_st_iosslider} iosSlideritem">
                <div class="container" style="background-image:url('{$banner.image_multi_lang}');">
    ...
    {if $slide.prev_next}
	<div id="iosSliderPrev_{$slide.id_st_iosslider_group}" class="iosSlider_prev none {if $slide.prev_next==2 || $slide.prev_next==4} hidden-xs {/if} {if $slide.prev_next==3 || $slide.prev_next==4} showonhover {/if} {if $slide.prev_next_style==1} ios_pn_rectangle {elseif $slide.prev_next_style==2} ios_pn_circle {/if}"><i class="icon-left-open-3"></i></div>
	<div id="iosSliderNext_{$slide.id_st_iosslider_group}" class="iosSlider_next none {if $slide.prev_next==2 || $slide.prev_next==4} hidden-xs {/if} {if $slide.prev_next==3 || $slide.prev_next==4} showonhover {/if} {if $slide.prev_next_style==1} ios_pn_rectangle {elseif $slide.prev_next_style==2} ios_pn_circle {/if}"><i class="icon-right-open-3"></i></div>
    {/if}
    {if $slide.pag_nav}
