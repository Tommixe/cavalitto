...
<section id="st_blog_related_article{$smarty.capture.column_slider}" class="block {if isset($column_slider) && $column_slider} column_block {/if} {if $hide_mob} hidden-xs {/if}">
    <h3 class="title_block {if (!isset($column_slider) || !$column_slider) && $title_position} title_block_center {/if}"><span>{l s='Related articles ' mod='stblogrelatedarticles'}</span></h3>
