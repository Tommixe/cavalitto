...
<aside class="st_blog_featured_article-column block">
	<p class="title_block">{l s='Featured articles' mod='stblogfeaturedarticles'}</p>
		...
		<ul class="pro_column_list">
            ...
            <li class="clearfix ">
    			...
    			<div class="pro_column_right">
    				<p class="s_title_block nohidden"><a href="{$link->getModuleLink('stblog', 'article',['id_blog'=>$blog['id_st_blog'],'rewrite'=>$blog['link_rewrite']])|escape:'html'}" title="{$blog.name|escape:'html':'UTF-8'}">{$blog.name|truncate:50:'...'|escape:html:'UTF-8'}</a></p><span class="date-add">{dateFormat date=$blog.date_add full=0}</span>
                </div>
            </li>
            {/foreach}
        </ul>
